...
def get_mem_usage_percent():
    try:
        ...
        return None
    physical_percent = usage_percent(
...
def check_disk():
    try:
        ...
        for i in p_list:
            ...
            return_dict[i] = (
                '%.2f' % (usage_percent(use, all),),
                ('%.2f' % (all * 1.0 / (1024 * 1000000))))
    except:
        return None
    return return_dict
...
def get_cpu_time():
    ...
    cpu_percentage = int(
        100.0 * (total_time - idle_time - iowait_time) / total_time)
    # compensate logic
...
def disk_io_Kbps():
    iostat = Popen(
        "iostat -d -k 1 2 | sed '/Device\|Linux\|^$/d' > /tmp/disk_io",
